...
- [ ] Creates `property_access_codes` table with suite NOT NULL DEFAULT '' (empty string for gates)
...
**Priority**: P3 | **Estimate**: 20min | **Depends on**: 1.1, 1.2

**Acceptance criteria**:
- [ ] Prefer empty tables + staff manual entry via `/ops/access-codes`
- [ ] If seeding approved: use per-property env vars (COTTAGE_GATE_CODE, MAIN_HOUSE_GATE_CODE)
- [ ] Do NOT blindly insert same PROPERTY_GATE_CODE into both cottage and main-house
...
- Prefer no seed, staff enter manually
...
- [ ] `getAccessCode(property, type, suite)` function queries DB (suite='' for gates)
- [ ] `resolveAccessCodes(property, suite?)` function returns `{ gateCode, doorCode, lockboxCode? }`
- [ ] DB-first logic: query DB for specific property+type+suite → if row exists, return code_value (even if empty)
- [ ] Env fallback logic: if NO DB row exists for that key, check global `PROPERTY_GATE_CODE` / `PROPERTY_DOOR_CODE`
- [ ] Fail-closed logic: if both empty, return `'[ASK STAFF]'`
- [ ] No cross-property env bleed: once DB row exists, env never used for that key
- [ ] Suite-specific lockbox codes resolve correctly
- [ ] No code values logged in function (authorized API responses over HTTPS exempt)
...
- [ ] Test: DB row exists → returns DB code_value (even if empty)
- [ ] Test: NO DB row, env set → returns env value
- [ ] Test: NO DB row, NO env → returns `'[ASK STAFF]'`
- [ ] Test: Multiple properties → correct scoping (cottage with DB row uses DB; main-house with NO row uses env)
- [ ] Test: Suite-specific lockbox → returns only that suite's code
- [ ] Test: Suite-specific lockbox, suite not found → returns `'[ASK STAFF]'`
- [ ] Test: Empty string suite for gates works correctly
...
**Acceptance criteria**:
- [ ] File `src/app/api/ops/access-codes/upsert/route.ts` exists
- [ ] POST endpoint accepts `{ property, code_type, suite, code }` (suite='' for gates, actual name for lockboxes)
...
- [ ] Inserts audit log entry to `access_code_audit_log` (metadata only, no code value)
- [ ] Returns success response (code value may be in HTTPS response for edit functionality)
...
**Acceptance criteria**:
- [ ] File `src/app/api/ops/access-codes/audit/route.ts` exists
- [ ] GET endpoint returns recent audit log (last 90 days)
- [ ] Validates staff auth
- [ ] Does NOT include code values (audit log stores metadata only)
...
**Acceptance criteria**:
- [ ] File `src/app/ops/access-codes/page.tsx` exists (server component)
- [ ] Checks staff auth, redirects if not authenticated
- [ ] File `src/app/ops/access-codes/AccessCodesManager.tsx` exists (client component)
- [ ] Fetches current codes on mount
- [ ] Groups codes by property
- [ ] Displays gate codes (suite='') and lockbox codes separately
- [ ] Inline edit UI with text inputs (masked by default, toggle to reveal)
- [ ] Suite field is free-text input for lockboxes (not hardcoded dropdown)
- [ ] Save button per row or per section
- [ ] Shows last updated timestamp and staff name for each code
- [ ] Displays audit log below (last 90 days, metadata only)
- [ ] Server logs never print actual codes (authorized HTTPS responses may contain plaintext)

**Implementation notes**:
- Use `useState` for edit mode
- Use `type="password"` for code inputs by default, toggle to `type="text"` on reveal
- Fetch codes: `GET /api/ops/access-codes` (new route or combine with upsert)
- Fetch audit: `GET /api/ops/access-codes/audit`
- On save: `POST /api/ops/access-codes/upsert` (with suite='' for gates)
...
**Acceptance criteria**:
- [ ] Test: Within time-gate, DB row exists → shown
- [ ] Test: Within time-gate, NO DB row, env set → env shown
- [ ] Test: Within time-gate, NO DB row, NO env → `[ASK STAFF]`
- [ ] Test: Outside time-gate → codes hidden (standard message)
- [ ] Test: Suite-specific lockbox code displayed correctly
- [ ] Test: Cottage with DB row + main-house without DB row → cottage uses DB, main-house uses env (no bleed)
- [ ] All tests pass
...
**Acceptance criteria**:
- [ ] `README.md` or `QUICK-START.md` mentions access codes SoR, path `/ops/access-codes`
...
**Acceptance criteria**:
- [ ] Manual test: Staff login, navigate to `/ops/access-codes`
- [ ] Manual test: Edit gate code (suite=''), save, verify audit log entry
- [ ] Manual test: Add lockbox code with free-text suite name, save
- [ ] Manual test: View guest portal with DB code set → code shown (within time-gate)
- [ ] Manual test: Clear DB code for cottage, verify env fallback works for cottage
- [ ] Manual test: Main-house with DB row does not use env fallback (no bleed)
- [ ] Manual test: Clear both, verify `[ASK STAFF]` shown
- [ ] Manual test: Generate welcome draft, verify code resolution
- [ ] Manual test: Server logs never print actual codes (check log files)

...
Staff open an Access Codes management page, view current codes for gates and lockboxes grouped by property, edit any code, save, and see the change reflected immediately. The system records who made the change and when. Live codes never appear in browser console, network inspector logs, or test fixtures.
...
**Independent Test**: Log in as staff, navigate to access codes page, change a gate code, save, and confirm the new code is stored with a change log entry. Verify console and network tabs show `****` for code values. Attempt to view codes as a guest and confirm they see `[ASK STAFF]` placeholder.

**Acceptance Scenarios**:

1. **Given** staff are authenticated, **When** they open the access codes management page, **Then** they see all properties with their gate and lockbox codes grouped clearly, and each code shows last-updated timestamp and staff name
2. **Given** staff edit a gate code for Cottage entrance (278 Blue Crane), **When** they save, **Then** the new code is stored in the database, a change log entry records their staff ID and timestamp, and the portal immediately uses the new code for time-gated display
3. **Given** staff edit a lockbox code for a specific room (e.g., Main House Suite 2), **When** they save, **Then** the system validates the code is not empty, stores it, and logs the change
4. **Given** a staff member is reviewing change history, **When** they view the audit log, **Then** each entry shows property, code type (gate/lockbox), old value (redacted), new value (redacted), staff name, and ISO timestamp
5. **Given** browser DevTools are open during code viewing/editing, **When** staff interact with the page, **Then** actual code values are never visible in console logs, network responses, or DOM inspector (shown as `****` or redacted)

---

### User Story 2 - Guest portal reads codes from DB with safe fallback (Priority: P1)

When a guest within the time-gate window (24h before check-in through checkout) views their portal, the system attempts to read gate and lockbox codes from the database. If DB codes exist, they are shown. If DB is empty, the system falls back to env vars `PROPERTY_GATE_CODE` / `PROPERTY_DOOR_CODE`. If both are empty, the portal displays `[ASK STAFF]` placeholder and never invents a code.
...
1. **Given** a booking within the time-gate window and gate code exists in DB, **When** the guest views their portal, **Then** the DB code is displayed and env var is ignored
2. **Given** a booking within the time-gate window and DB gate code is empty but `PROPERTY_GATE_CODE` env var is set, **When** the guest views their portal, **Then** the env var code is displayed as fallback
3. **Given** a booking within the time-gate window and both DB and env are empty, **When** the guest views their portal, **Then** the access codes section shows `[ASK STAFF]` placeholder with a message to contact staff
...
5. **Given** a room/suite has a lockbox code in DB, **When** the guest portal loads that booking, **Then** the lockbox code for their specific suite is displayed (not codes for other suites)
...
- Template generation occurs during DB query timeout → falls back to env, then `[ASK STAFF]`
...
- Guest attempts to directly access staff access codes management URL → authentication middleware redirects to guest portal or login
...
- **FR-001**: System MUST store property access codes in a dedicated `property_access_codes` table with columns for property identifier, code type (gate_pinpad or lockbox), suite identifier (nullable, required for lockbox), code value (encrypted at rest), last updated timestamp, and last updated by staff ID
- **FR-002**: System MUST provide a staff-only UI route (e.g., `/staff/access-codes`) to view and edit all access codes, grouped by property and code type
...
- **FR-006**: Guest portal API route (`/api/guest-portal/[code]`) MUST read access codes from DB first, fall back to env vars `PROPERTY_GATE_CODE` / `PROPERTY_DOOR_CODE` if DB is empty, and return `[ASK STAFF]` if both are empty
...
- **FR-011**: System MUST redact actual code values in all client-side logs, browser DevTools network responses, and console output, showing `****` or similar placeholder
- **FR-012**: System MUST prevent browser autofill for access code input fields using appropriate HTML attributes
- **FR-013**: Staff access codes management route MUST require authenticated staff session; guest tokens or unauthenticated access MUST be denied
...
- **FR-015**: Lockbox codes MUST be suite-specific, allowing different codes for different rooms/suites at the same property
...
- **FR-018**: Seed data script or initial migration MUST pre-populate DB codes from current env vars if staff provide `APPROVE SEED FROM ENV` confirmation
- **FR-019**: System MUST never log, print, or display live access codes in test fixtures, automated test output, CI logs, or development console
...
- **SC-004**: Zero live access codes appear in browser DevTools, test fixtures, CI logs, or PR descriptions (all examples use `REDACTED` or `****`)

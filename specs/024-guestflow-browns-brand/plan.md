...
**GFM LOCK (25 Sep via CoS)**: Phase 2b guest portal covers exactly six product-sensitive surfaces (visual restyle only): (1) magic-link entry + expired/invalid states, (2) access codes display (pins/lockbox SoR), (3) WiFi SoR / network_ask_staff, (4) stay summary facts, (5) check-in/arrival chips/CTAs, (6) contact handoff to +27600200825. Do NOT change copy logic, gates, or data — remap existing UI 1:1 onto Phase 0 tokens.

**DESIGN ADDENDUM (24 Sep READY)**: Authoritative visual SoT received from Design team. Guest portal mockups: 01-guest-stay-hub-proposed.png (authenticated hub with placeholder labels), 02-guest-invalid-proposed.png (Unable to Access error state). Replace pale cool-blue wash with warm muted #F6F5F3; multicolor section headers with navy/gold restraint; cyan links with primary navy; add Browns CI mark; Playfair H1 on page titles/empty-states; Montserrat dense UI. Authenticated hub sections: Stay summary, Wi-Fi, Access codes (time-gated), Parking, Directions, House rules, Contact, Rebook, Loading/Unable states.

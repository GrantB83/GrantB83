...
- **Shopping split** (2026-08-23): autonomous on the 7th. **Emails first** (hub and thebrownsusa), then Monarch receipt images only for leftovers. AI assigns each item description to a line on the **Line items** tab. Prefer an existing plan line over asking Grant. Leftover only if merchant and purpose are still unknown. Grant’s answers write the tab. Main Budget shows **category totals only** (pulled from that tab). Park still-open Amazon rows on `Shopping (bundle)`. Flag tax-review; do not decide deductibility or file (`N2`). Leftover list = merchant, description, date, source — no amounts unless Grant asks. Skip pharmacy/medical item text (`N3`).
- **Dry-run leftover** (2026-08-23): Finance folder has `Budget — Line items AI 2026-08-23` (64 AI allocations, no amounts) and `Budget — Leftover for Grant 2026-08-23 — 1 left` (Brown & Grant only). Empty draft and the 66-row leftover were trashed. Live Budget was not edited. Add two plan lines there: `Austin utilities (bundle)`, `Shopping (bundle)`. Family Bot writes Line items on the 7th.

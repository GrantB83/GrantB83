...
- **GuestFlow Sprint 4 #4 WA Web full bodies + source names (Ship B)** (26 Sep 2026, `cursor/wa-web-full-bodies-dbab` on `main@31c0e8f` / #245): Observe ingest no longer writes `[metadata-only]` / `[body unavailable]`; empty observe skips the row. Backfill updates WhatsApp Web sentinels in place (14-day or open sentinel). Unmatched temps persist contact/push/chat title as `bookerName` when the source has one. Cloud↔Web same guest line stays one row. Redirect ON; Approve&Send human; From `+27600200825`. Ritual removed: opening WhatsApp Web to read what the guest said because the soft inbox only showed a sentinel. Artefact: `apps/guestflow/docs/WA-WEB-FULL-BODIES.md` + Preview `/?thread=28`. Spec Kit `specs/030-wa-web-full-bodies/`. MERGE HOLD until GFM ACCEPT. No Production Turso apply. No send. No composer / Link-modal / Cloud conversion of `+27836458313`.

...
- **USA Budget close** (2026-08-23): 7th of each month at **07:00 America/Chicago**. **Family closes** (not remind-only): export Monarch if the session is live, map transactions, write Budget vs Actual with amounts, file the statement if present. Grant is pinged only for a dead session, a missing statement, or Status=ask (`FAMILY CLOSES: yes`). Liana **edits** the sheet (`LIANA SHEET: yes`). Forecast unchanged. First close = dry-run now; repeat 7 Sep 2026 07:00 CT. Daily Family digest amend still waits.
...
| 10d | Budget / bills | **Family closes on the 7th** | Paste Family close-block amend; Grant answers Brown & Grant; add 2 plan lines | hunt → due list | first live close 7 Sep 07:00 CT |

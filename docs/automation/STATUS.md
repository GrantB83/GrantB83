...
- **GuestFlow Sprint 4 #4 WA Web full bodies + source names (Ship B)** (26 Sep 2026, `cursor/wa-web-full-bodies-dbab` on `main@31c0e8f` / #245): Observe ingest no longer writes `[metadata-only]` / `[body unavailable]`; empty observe skips the row. Backfill updates WhatsApp Web sentinels in place (14-day or open sentinel). Unmatched temps persist contact/push/chat title as `bookerName` when the source has one. Cloud↔Web same guest line stays one row. **Read-path remedia:** Turso wrapper no-store fetch + plain rows + call-site args so staff GET/UI `/?thread=28` reads live `message_text` (ids 51/61/70/75); replace clears `metadata.metadataOnly`. Redirect ON; Approve&Send human; From `+27600200825`. S9 QA job-script: open thread 28 + peers; read real text + names. S10 evidence: Prod `?thread=28` before/after (or Preview of the same data) + `whatsapp_web` metadata-only inbox count (`specs/030-wa-web-full-bodies/EVIDENCE.md`). Ritual removed: opening WhatsApp Web to read what the guest said because the soft inbox only showed a sentinel. Artefact: `apps/guestflow/docs/WA-WEB-FULL-BODIES.md`. Spec Kit `specs/030-wa-web-full-bodies/`. **MERGE HOLD until GFM ACCEPT after QA job-script.** No Production Turso apply. No send. No composer / `#245` / Redirect flip / Cloud conversion of `+27836458313`.
- **GuestFlow Sprint 3 S Ultra-only Phase 1 drafts** (25 Sep 2026, `cursor/guestflow-ultra-only-6420` rebased on `main@764b2ea` / #242): Phase 1 batch worker no longer calls OpenAI `chat.completions`. Cursor Ultra CA is the only model path. Live run without dry-run / drafts-file / injected generator **refuses the batch** (zero claims). `OPENAI_API_KEY` ignored; docs must not tell anyone to set it for Production Phase 1. Webhook still enqueue-only. Approve&Send human. Redirect + From `+27600200825` unchanged. T+U sticky/mobile inbox from #242 preserved. Ritual removed: provisioning a third-party chat-completion key so Phase 1 drafts can exist. Artefact: `apps/guestflow/docs/CURSOR-ULTRA-BATCH-LAUNCH.md` + fail-closed worker. Spec Kit `specs/027-sprint3-phase1-ultra-only/`. Supersedes stale PR #201. GFM ACCEPTED on #243. No Production Turso. No send. No go-live flip.
- **GuestFlow Sprint 3 T+U sticky header + mobile width** (25 Sep 2026, merged `main@764b2ea` / #242): Staff navy header + Redirect banner/control are one fixed stack (`[data-ops-chrome]`) so they stay on screen while inbox list/thread panes scroll. Chrome height is measured once (`useInboxChromeOffset` / `--ops-chrome-height`); inbox-lock zeroes `main` padding so #235 floors are not double-counted. Phone ~390 CSS px list + thread use 8px gutters and a stacked thread header so preview / upper box are ≥90% of shell width. `LIST_SCROLL_KEY`, Approve&Send, Redirect ON, WA From `+27600200825` unchanged. Ritual removed: scrolling back to the top to see Redirect, then pinch-zooming a sliver preview on phone. Artefact: `/?fixture=1` + `specs/027-sprint3-sticky-header-mobile/`. No auto-send. No redirect go-live. No stay@ / Ultra.

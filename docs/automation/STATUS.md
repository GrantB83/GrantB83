...
- **GuestFlow Phase 1 Cursor Ultra-only batch drafts** (21 Sep 2026, `cursor/guestflow-ultra-only-cfea`): Realigned Phase 1 off OpenAI onto Cursor Ultra exclusive. OpenAI `chat.completions` path removed. Cursor Ultra CA generates drafts using its own model, no OPENAI_API_KEY required in Production. Batch worker fails closed if not launched BY Coding/Grok as Cursor Ultra CA task. Vercel/webhook = enqueue `draft_jobs` only (verified; no LLM in Vercel). Batch runner: Coding launches Cursor Ultra CA → CA model writes drafts → `POST /api/drafts/upsert` with `DRAFT_WORKER_SECRET` + `draft_source=llm`. Docs updated: no "set OPENAI_API_KEY for Production". Approve&Send + confirmToken + OUTBOUND redirect unchanged. Caps still: ≥5 or 20min; 07:00–21:00 SAST; ≤6 batches/day; intent allowlist. Tests updated for Ultra-only path. Ritual removed: OpenAI dependency for Production pilot. Artefact: Cursor Ultra-only batch worker. Phase 1 Ultra batch is NEW PR (includes #199 A&D + #200 nav as base). No merge yet, no Production secrets set. Awaiting Vercel Preview green + Grant CLEAR.

...
<<<<<<< HEAD
- **GuestFlow Sprint 2 data-fixes** (25 Sep 2026, `cursor/sprint2-data-fixes-27a6`): Shared active-guest filter (cancelled + owner `BLOCK`) on daily-brief / enqueue / checkin-status with SAST dates; lockbox-only Cottage 278 vs Main House 279 resolver (fail closed, no suite-name substrings); Inbox GET read-only; threads only when a real message exists; `needs_attention` = unanswered inbound only; exceptions + webhook inserts use live `guest_tickets` columns; demo lodge seed removed; `/api/today-stats` deleted. Item Q (rate-cards) dropped. Ritual removed: Grant eyeballing cancelled/BLOCK rows and guessing Cottage vs Main House from suite names, plus Inbox creating empty needs-attention threads. Artefact: Preview daily brief + Exceptions this week. Spec Kit `specs/017-sprint2-data-fixes/` (specify → plan → tasks → implement → converge). No Production Turso writes. No send. Do not merge until gate.
=======
- **GuestFlow staff user management** (24 Sep 2026, `cursor/staff-user-management-3989`): Named staff users (**email** + password, optional display name), identical full access (no roles/RBAC). Per-user sessions replace the shared `staff_auth` cookie. Users page at `/ops/users`. Logout in nav. First user seeded from `GUESTFLOW_BOOTSTRAP_EMAIL` / `GUESTFLOW_BOOTSTRAP_PASSWORD` when the table is empty. Legacy `legacy@guestflow.local` + `STAFF_PASSWORD` stays on until `GUESTFLOW_LEGACY_LOGIN=0`. Migrate script included — **not applied to Production**. Ritual removed: sharing one staff password and asking Grant to rotate it. Artefact: `/staff-login`, `/ops/users`, `apps/guestflow/docs/STAFF-USERS.md`. Do not merge; no Production deploy.
>>>>>>> 6f57906 (docs(guestflow): switch 016 staff login identifier to email)

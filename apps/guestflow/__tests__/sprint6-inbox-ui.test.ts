...
describe('Sprint 6 inbox UI contract', () => {
  ...
  it('uses exact SoR toolbar tooltip copy and pop-out', () => {
    ...
    expect(composer).toContain('aria-describedby="composer-attach-disabled"')
    expect(composer).toContain('aria-disabled="true"')
    expect(composer).not.toMatch(/title=\{ATTACH_DISABLED_REASON\}/)
    expect(composer).not.toContain('describedBy={ATTACH_DISABLED_REASON}')
    expect(tooltip).toContain('onFocusCapture={showNow}')

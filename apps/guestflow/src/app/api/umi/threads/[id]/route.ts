...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json({ success: true, thread }, { headers })
  } catch (error) {
...
export async function PUT(
  ...
  try {
    ...
    if (!Number.isFinite(threadId) || !text) {
      return NextResponse.json(
        { success: false, error: 'thread id and text are required' },
        { status: 400, headers }
      )
    }
    const db = await getDbAsync()
    ...
    if (!latest) {
      return NextResponse.json(
        { success: false, error: 'No message to attach draft' },
        { status: 400, headers }
      )
    }
    await db
    ...
    await db
      ...
      .run(threadId)
    return NextResponse.json({ success: true, threadId, draftSource: 'human' }, { headers })
  } catch (error) {
